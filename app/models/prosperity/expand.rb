...
class Prosperity::Expand < Card
  costs 7
  action
  card_text "Action (cost: 7) - Trash a card from your hand. " +
  ...
  def determine_controls(player, controls, substep, params)
    ...
    when "take"
      valid_piles = game.piles.map do |pile|
        (pile.cost <= (params[:trashed_cost].to_i + 3)) and not pile.empty?
      end
  ...
  def resolve_trash(ply, params, parent_act)
    ...
    parent_act.children.create!(:expected_action => "resolve_#{self.class}#{id}_take;trashed_cost=#{trashed_cost}",
                               :text => "Take a replacement card with Expand",
                               :player => player,
                               :game => game)
