class Player < ActiveRecord::Base
  ...
  def draw_cards(num, reason = nil)
    ...
    deck_cards = cards.deck.to_a
    (0..[num, cards.deck.size].min - 1).each do |n|
      card = deck_cards.shift
      if cards.hand.empty?
        card.position = 0
      else
  ...
  def cards_for_decklist(html = true)
    deck = cards.unscope(:order).group(:type).count.sort do |gp_a, gp_b|
      type_a = gp_a[0]
      type_b = gp_b[0]
      a = cards.find_by(type: type_a)
      b = cards.find_by(type: type_b)
