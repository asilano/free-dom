module GameEngine
  module Helpers
    def self.gain_card_from_supply(game_state, player:, card_class:, to: :deck)
      pile = game_state.find_pile_by_top_card { |c| c.is_a? card_class }
      card = pile&.cards&.first
      game = game_state.game

      if card
        game.current_journal.histories << History.new("#{player.name} gained a #{card_class.readable_name}#{" to their #{to}" unless to == :deck}.",
                                                      player:      player,
