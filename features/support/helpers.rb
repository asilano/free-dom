AfterStep do |scenario|
  ...
  if @skip_card_checking == 0
    @test_players.each do |name, test_player|
      ...
      assert_same_elements @play_contents[name], player.cards.in_play.map(&:readable_name), "#{name}'s cards in play didn't match"
      assert_same_elements @enduring_contents[name], player.cards.enduring.map(&:readable_name), "#{name}'s enduring cards didn't match"

      assert_equal @deck_contents[name], player.cards.deck.map(&:readable_name), "#{name}'s deck didn't match"
    end
  else
